...
std::pair<double, double> evaluate(GPTModel *model,
  ...
  for (const auto &batch : *test_data_loader) {
    ...
    // logits : [B, T, C]
    // 计算正确率
    auto N = logits.count(0, 1);
    auto C = logits.size(-1);

    if (model->ctx()->device.is_blas()) {
      vec::mat_softmax_forward_blas(logits.mutable_data_as<float>(), N, C);
    } else {
      vec::mat_softmax_forward_native(logits.mutable_data_as<float>(), N, C);
    }
...
train(GPTModel *model, const std::string &model_dir,
