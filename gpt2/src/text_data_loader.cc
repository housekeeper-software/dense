...
dense::Tensor LoadPreProcessedData(const std::string &filename) {
  ...
  return dense::Tensor::from_blob(dense::DType::kInt64, {size}, &all_tokens[0])
      .clone();
}

class TextDataset : public dense::Dataset {
public:
  TextDataset(dense::Tensor data_sequence, int64_t context_length)
      : data_sequence_(data_sequence), context_length_(context_length) {}
  ...
  dense::Example get(size_t index) override {
    ...
    size_t end_idx = index + context_length_ + 1;
    ...
    size_t element_size = data_sequence_.element_size();

    auto data_ptr =
        data_sequence_.mutable_data_as<uint8_t>() + start_idx * element_size;

    auto x_input = dense::Tensor::from_blob(dense::DType::kInt64,
                                            {context_length_}, data_ptr);
    data_ptr += element_size;
    auto y_target = dense::Tensor::from_blob(dense::DType::kInt64,
                                             {context_length_}, data_ptr);
...
std::shared_ptr<dense::Dataset> LoadDataset(const std::string &filename,
  ...
  auto tensor = LoadPreProcessedData(filename);
  if (!tensor.is_defined())
    return nullptr;
  return std::make_shared<TextDataset>(tensor, context_length);
}
